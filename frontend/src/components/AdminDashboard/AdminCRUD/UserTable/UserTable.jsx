...
const UserTable = () => {
  ...
  const columnName = [
    ...
    },
    {
      ...
      render: (rowData) => (
        <div>
        <Tooltip title='Edit'><Link  href={`/admin/list-users/${rowData._id}/edit`} style={{ paddingRight:'15px'}}><EditIcon color='action'/></Link></Tooltip> 
        
        <Tooltip title='Delete'><Link onClick={()=>handleRowDelete(rowData._id)}><DeleteIcon color='secondary' /> </Link></Tooltip>
        </div>
      ),
    },
  ]
  ...
  const { error: errorDeleting, success: deleteSuccess } = userDelete
  ...
  return (
    ...
    <ToolbarUser AddUser={'/admin/list-users/add'}>
    </ToolbarUser>
    </Card>
    <Grid item xs={12}>
      {loadingUserList ? (
